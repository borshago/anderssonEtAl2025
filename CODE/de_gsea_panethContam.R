...
pdf("RESULTS/PANETH_CONTAM/sampleCorrelationHeatmap.pdf",6,5)
pheatmap(sampleDistMatrix,
         clustering_distance_rows=sampleDists,
         clustering_distance_cols=sampleDists,
         col=colors)
...
pdf("RESULTS/PANETH_CONTAM/pca.pdf",5,5)
ggplot(pcaData, aes(PC1, PC2, color=population, shape=mouse)) +
  geom_point(size=3) + scale_shape_manual(values = 1:nlevels(sampleTable$mouse)) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
...
pdf("RESULTS/PANETH_CONTAM/cooks.pdf",5,5)
boxplot(log10(assays(dds)[["cooks"]]), range=0, las=2)
dev.off()
...
pdf("RESULTS/PANETH_CONTAM/dispersions.pdf",5,5)
plotDispEsts(dds)
dev.off()
...
pdf("RESULTS/PANETH_CONTAM/scaterPlot.pdf",10,5)
...
write.table(res,"RESULTS/PANETH_CONTAM/degs.txt",quote = F,sep = "\t",row.names = T,col.names = NA)

# Visualize expression of DE genes

#res <- read.table("RESULTS/PANETH_CONTAM/degs.txt",header=T,stringsAsFactors=F,sep="\t",row.names=1)
...
pdf(paste0("RESULTS/PANETH_CONTAM/DE/",degSymbols[i],"_normcounts.pdf"),6,4)
...
write.table(gseaRes,"RESULTS/PANETH_CONTAM/GSEA/gsea_camera_results.txt",quote = F,sep = "\t",row.names = T,col.names = NA)


### Top candidates and visualization ###

#library(limma)
#gseaRes <- read.table("RESULTS/PANETH_CONTAM/GSEA/gsea_camera_results.txt",stringsAsFactors=F)
res <- read.table("RESULTS/PANETH_CONTAM/degs.txt",stringsAsFactors=F)
...
{
sink("RESULTS/PANETH_CONTAM/GSEA/gsea_topCandidates.txt")
print(leading)
sink()
}
...
for (i in 1:length(gsets)) {
  pdf(paste0("RESULTS/PANETH_CONTAM/GSEA/geneset",i,"_stat.pdf"),5,4)
